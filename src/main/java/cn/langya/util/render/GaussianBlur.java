...
import cn.langya.util.misc.MathUtil;
...
public class GaussianBlur extends MinecraftInstance {
    ...
    private static final ShaderUtil gaussianBlur = new ShaderUtil("liquidbounce/shader/gaussian.frag");

    private static Framebuffer framebuffer = new Framebuffer(1, 1, false);

    private static void setupUniforms(float dir1, float dir2, float radius) {
        gaussianBlur.setUniformi("textureIn", 0);
        gaussianBlur.setUniformf("texelSize", 1.0F / (float) mc2.displayWidth, 1.0F / (float) mc2.displayHeight);
    ...
    public static void endBlur(float radius, float compression) {
        ...
        glBindTexture(GL_TEXTURE_2D, mc2.getFramebuffer().framebufferTexture);

        ShaderUtil.drawQuads();
        framebuffer.unbindFramebuffer();
        gaussianBlur.unload();

        mc2.getFramebuffer().bindFramebuffer(false);
        gaussianBlur.init();
