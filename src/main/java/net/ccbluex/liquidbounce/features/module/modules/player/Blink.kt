...
class Blink : Module() {
    ...
    private val hytnewvalue = BoolValue("CancelC0F", false)
    ...
    override fun onEnable() {
        ...
        if (!pulseValue.get()) {
            val faker: IEntityOtherPlayerMP =
                classProvider.createEntityOtherPlayerMP(mc.theWorld!!, thePlayer.gameProfile)
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (classProvider.isCPacketPlayerPosition(packet) || classProvider.isCPacketPlayerPosLook(packet) ||
            ...
            classProvider.isCPacketEntityAction(packet) || classProvider.isCPacketUseEntity(packet) || (hytnewvalue.get() && event.packet.unwrap() is CPacketConfirmTransaction)) {
            event.cancelEvent()
            packets.add(packet)
        }
    }
