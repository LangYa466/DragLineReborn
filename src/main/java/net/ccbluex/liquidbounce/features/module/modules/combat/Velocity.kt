...
class Velocity : Module() {
    ...
    private val modeValue = ListValue("Mode", arrayOf("AAC4","Simple", "AAC", "AACPush", "AACZero",
            "Reverse", "SmoothReverse", "Jump", "Glitch","Hyt"), "Simple")
    ...
    // Hyt
    private val packetSprintValue = BoolValue("PacketSprint", true)
    private val repeatValue = IntegerValue("Repeat", 8, 1, 12)
        ...
        when (modeValue.get().toLowerCase()) {
            ...
            "hyt" -> {
                ...
                repeat(repeatValue.get()) {
                    mc2.player.connection.sendPacket(
                        CPacketUseEntity(
                            (LiquidBounce.moduleManager.getModule(KillAura::class.java) as KillAura).target!!.unwrap(),
                            EnumHand.MAIN_HAND
                        )
                    )
                    mc.netHandler.addToSendQueue(classProvider.createCPacketUseEntity((LiquidBounce.moduleManager.getModule(KillAura::class.java) as KillAura).target!!.asEntityLivingBase(), ICPacketUseEntity.WAction.ATTACK))
