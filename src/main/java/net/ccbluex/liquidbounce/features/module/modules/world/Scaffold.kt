...
open class Scaffold : Module() {
    ...
    // Basic stuff
    val sprintValue = ListValue("Sprint", arrayOf("Vanilla", "Hyt"), "Vanilla")
    ...
    private val blocksToEagleValue =
        IntegerValue("BlocksToEagle", 0, 0, 10)// .displayable { !eagleValue.get().toLowerCase().equals("off") }
    private val edgeDistanceValue =
        FloatValue("EagleEdgeDistance", 0.2f, 0f, 0.5f)// .displayable { !eagleValue.get().toLowerCase().equals("off") }

    // Expand
    private val expandLengthValue =
        IntegerValue("ExpandLength", 5, 1, 6)// .displayable { modeValue.get().toLowerCase().equals("expand") }
    ...
    private val rotationModeValue =
        ListValue("RotationMode", arrayOf("Normal", "Static", "StaticPitch", "StaticYaw", "Off"), "StaticYaw")
    private val silentRotation = BoolValue("SilentRotation", true)
    private val keepRotationValue = BoolValue("KeepRotation", true)
    private val keepLengthValue = IntegerValue("KeepRotationLength", 7, 0, 20)// .displayable { keepRotationValue.get() }
    private val staticPitchValue =
        FloatValue("StaticPitchOffset", 86f, 70f, 90f)// .displayable { rotationModeValue.get().equals("StaticPitch") }
    private val staticYawOffsetValue =
        FloatValue("StaticYawOffset", 0f, 0f, 90f)// .displayable { rotationModeValue.get().equals("StaticYaw") }
    ...
    private val zitterModeValue =
        ListValue("ZitterMode", arrayOf("Teleport", "Smooth"), "Teleport")// .displayable { zitterValue.get() }
    private val zitterSpeed = FloatValue("ZitterSpeed", 0.13f, 0.1f, 0.3f)// .displayable { zitterValue.get() }
    private val zitterStrength = FloatValue("ZitterStrength", 0.072f, 0.05f, 0.2f)// .displayable { zitterValue.get() }

    // Game
    private val timerValue = FloatValue("Timer", 1f, 0.1f, 10f)
    private val speedModifierValue = FloatValue("SpeedModifier", 1f, 0f, 2f)
    private val slowValue = BoolValue("Slow", false)
    private val slowSpeed = FloatValue("SlowSpeed", 0.6f, 0.2f, 0.8f)// .displayable { slowValue.get() }
    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        if (noBlockDisablerValue.get()) {
            if (blocksAmount == 0) {
                ...
                moduleManager[GroundTelly::class.java].state = false
            }
        }

        if(sprintValue.get().equals("Vanilla")) {
            mc2.gameSettings.keyBindSprint.pressed = true
        } else {
            mc2.gameSettings.keyBindSprint.pressed = mc2.player.onGround
        }

        if (autoJumpValue.get() && (!noBlockDisablerValue.get() || blocksAmount == 0)) {
        ...
        get() {
            ...
            for (i in 36..44) {
                ...
                if (itemStack != null && classProvider.isItemBlock(itemStack.item)) {
                    ...
                    if (heldItem != null && heldItem == itemStack || !InventoryUtils2.BLOCK_BLACKLIST.contains(block) && !classProvider.isBlockBush(
                            block
                        )
