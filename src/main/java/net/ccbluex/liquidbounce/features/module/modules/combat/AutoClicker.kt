...
class AutoClicker : Module() {
    ...
    private val breakBlockValue = BoolValue("LeftBreakBlocks", false)
    private val onlyBlockValue = BoolValue("RightOnlyBlocks", false)
    ...
    fun onRender(event: Render3DEvent) {
        ...
        if (mc.gameSettings.keyBindUseItem.isKeyDown && !mc.thePlayer!!.isUsingItem && rightValue.get() &&
            ...
            if (!onlyBlockValue.get() || classProvider.isItemBlock(mc.thePlayer!!.heldItem?.item)) mc.gameSettings.keyBindAttack.onTick(
                mc.gameSettings.keyBindUseItem.keyCode
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (jitterValue.get() && (leftValue.get() && mc.gameSettings.keyBindAttack.isKeyDown && mc.playerController.curBlockDamageMP == 0F
            ...
            if (Random.nextBoolean()) thePlayer.rotationYaw += if (Random.nextBoolean()) -RandomUtils.nextFloat(
                0F,
                1F
