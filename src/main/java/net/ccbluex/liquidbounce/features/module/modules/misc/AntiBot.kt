...
import net.ccbluex.liquidbounce.api.minecraft.util.IChatComponentText
...
class AntiBot : Module() {
    ...
    private val hytMessageCheckValue = BoolValue("HytMessageCheck", true)
    ...
    private val hytBotsNames = mutableListOf<String>()


    private fun addbot(name: String, delay: Long) {
        Thread {
            val checkTime = MSTimer()
            hytBotsNames.add(name)
            LiquidBounce.hud.addNotification(Notification("AntiBot","$name added to the list", NotifyType.INFO))
            if(checkTime.hasTimePassed(delay)) {
                hytBotsNames.remove(name)
                LiquidBounce.hud.addNotification(Notification("AntiBot","$name removed from the list", NotifyType.INFO))
            }
        }
    }
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if(event.packet.unwrap() is CPacketChatMessage) {
            val message = (event.packet.unwrap() as CPacketChatMessage).message
            if(hytMessageCheckValue.get()) {
                val matchers = mutableListOf<Matcher>()
                matchers.add(Pattern.compile("杀死了 (.*?)\\(").matcher(message))
                matchers.add(Pattern.compile("起床战争>> (.*?) (\\(((.*?)死了!))").matcher(message))
                matchers.add(Pattern.compile("杀死了 (.*?)\\[").matcher(message))
                matchers.add(Pattern.compile("起床战争>> (.*?) \\[").matcher(message))
    ...
    companion object {
        ...
        fun isBot(entity: IEntityLivingBase): Boolean {
            ...
            for (hytBotsName in antiBot.hytBotsNames) {
                return hytBotsName == entity.displayName!!.formattedText
            }
