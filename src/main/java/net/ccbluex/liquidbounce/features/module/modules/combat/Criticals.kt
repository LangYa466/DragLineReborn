...
class Criticals : Module() {
    ...
    val modeValue = ListValue("Mode", arrayOf("NewPacket","Packet", "NcpPacket", "NoGround", "Hop", "TPHop", "S08", "Jump", "LowJump"), "packet")
    ...
    fun onAttack(event: AttackEvent) {
        if (classProvider.isEntityLivingBase(event.targetEntity)) {
            ...
            when (modeValue.get().toLowerCase()) {
                ...
                "s08" -> {
                    mc.netHandler.addToSendQueue(classProvider.createCPacketPlayerPosition(x, y + 0.01, z, true))
                }
                "packet" -> {
