...
@ModuleInfo(name = "KillAura", category = ModuleCategory.COMBAT,
    keyBind = Keyboard.KEY_R, description = "Auto Attack")
...
class KillAura : Module() {
    ...
    // Modes
    private val priorityValue = ListValue("Priority", arrayOf("Health", "Distance", "Direction", "LivingTime", "Armor"), "Health")
    ...
    private val blockModeValue = ListValue("AutoBlock", arrayOf("Off", "Packet", "AfterTick","Nore"), "Packet")
                    ...
                    if (target != null) {
                        lastTarget = if (lastTarget == null) {
                            ...
                            if (lastTarget!!.health <= 0) {
                                mc2.connection!!.handleSpawnGlobalEntity(SPacketSpawnGlobalEntity(EntityLightningBolt(mc2.world,
    ...
    private fun updateHitable() {
        ...
        if (raycastValue.get()) {
            ...
            if (raycastValue.get() && raycastedEntity != null && classProvider.isEntityLivingBase(raycastedEntity)
                && (LiquidBounce.moduleManager[NoFriends::class.java].state || !(classProvider.isEntityPlayer(raycastedEntity))))
