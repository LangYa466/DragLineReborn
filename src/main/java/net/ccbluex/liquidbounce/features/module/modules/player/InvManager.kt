...
import net.ccbluex.liquidbounce.utils.InventoryUtils2
...
class InvManager : Module() {
    ...
    private val sortSlot2Value = ListValue("SortSlot-2", items, "Pickaxe")// .displayable { sortValue.get() }
    private val sortSlot3Value = ListValue("SortSlot-3", items, "Gapple")// .displayable { sortValue.get() }
    private val sortSlot4Value = ListValue("SortSlot-4", items, "Block")// .displayable { sortValue.get() }
    private val sortSlot5Value = ListValue("SortSlot-5", items, "Food")// .displayable { sortValue.get() }
    private val sortSlot6Value = ListValue("SortSlot-6", items, "None")// .displayable { sortValue.get() }
    private val sortSlot7Value = ListValue("SortSlot-7", items, "Bow")// .displayable { sortValue.get() }
    private val sortSlot8Value = ListValue("SortSlot-8", items, "Axe")// .displayable { sortValue.get() }
    private val sortSlot9Value = ListValue("SortSlot-9", items, "Pearl")// .displayable { sortValue.get() }
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (!InventoryUtils2.CLICK_TIMER.hasTimePassed(delay) || (mc2.player.hurtTime > 0 && noCombatValue.get()) || (!classProvider.isGuiInventory(
                mc.currentScreen
            ) && invOpenValue.get()) || noMoveValue.get() && isMoving || thePlayer.openContainer != null && thePlayer.openContainer!!.windowId != 0
        ) return

        while (InventoryUtils2.CLICK_TIMER.hasTimePassed(delay)) {

            // Find best armor
            val armorPieces = IntStream.range(0, 36).filter { i: Int ->
                val itemStack = mc.thePlayer!!.inventory.getStackInSlot(i)
                itemStack != null && (classProvider.isItemArmor(itemStack.item) && (!ignoreChainArmorValue.get() || !(itemStack.unlocalizedName == "item.helmetChain" || itemStack.unlocalizedName == "item.leggingsChain"))) && (i < 9 || System.currentTimeMillis() - itemStack.itemDelay >= itemDelayValue.get())
            }.mapToObj { i: Int ->
                ArmorPiece(
            ...
            for (i in 0..3) {
                ...
                val armorSlot = 3 - i
                val oldArmor = ArmorPiece(
                    mc.thePlayer!!.inventory.armorItemInSlot(armorSlot), -1
                )
    ...
    fun isUseful(itemStack: IItemStack, slot: Int): Boolean {
        return try {
            ...
            if (classProvider.isItemSword(item) || classProvider.isItemTool(item)) {
                ...
                item
            ) && !InventoryUtils2.SKYWAR_BLOCK_BLACKLIST.contains(item!!.asItemBlock().block)) || classProvider.isItemBed(
                item
            ...
            "block" -> {
                thePlayer.inventory.mainInventory.forEachIndexed { index, stack ->
                    if (stack != null) {
                        ...
                        if (classProvider.isItemBlock(item) && !InventoryUtils2.BLOCK_BLACKLIST.contains(item.asItemBlock().block) && !type(
                                index
