...
class ScoreboardElement(
    ...
    private val allowedDomains = arrayOf(
        ".ac",
        ".academy",
        ".accountant",
        ".accountants",
    ...
    fun shadow() {
        ...
        val objective = currObjective ?: worldScoreboard.getObjectiveInDisplaySlot(1)
        ...
        Gui.drawRect(l1 - 7, -5, 9, maxHeight + fontRenderer.fontHeight + 5, Color(0, 0, 0, 255).rgb)
//Shadow!!!!
                ...
                for (domain in allowedDomains) {
                    if (name.contains(domain, true)) {
                        name = when (serverValue.get().toLowerCase()) {
                            "clientname" -> LiquidBounce.CLIENT_NAME + " Main"
                            else -> "null"
                        }
                        listColor = Color(234, 246, 100).rgb
                        break
                    }
                }
            }
    ...
    private fun textColor() = Color(
        textRedValue.get(), textGreenValue.get(),
        textBlueValue.get()
    )
}